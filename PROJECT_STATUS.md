...
- **UI**: 기존 기부한/받은 2탭 → **3탭**(기부한/받은/**받은 편지 N**). 편지 카드는 상세의 기부자 편지와 같은 편지지 톤(주황 `#FFF1E5`/`#B96A25`)으로 보낸 사람·물품·날짜(YYYY.MM.DD)·**본문을 카드에 그대로 펼쳐** 보여주는 **자기완결형 카드**(링크 이동 없음). 빈 상태 문구 추가.
  - ⚠️ **링크 이동을 뺀 이유**: 초기엔 카드→물품 상세로 링크했는데, 상세엔 "받기 전 잠금"된 **기부자 편지**(내가 쓴 별개 편지)가 떠서 "받은 편지가 잠겼다"는 오해를 유발함. 받은 편지(수혜자 답장)는 카드 본문에 이미 다 있으므로 이동 불필요. (기부자 편지의 소유자 열람 잠금 해제는 후속 과제 #2)
  - **본문 접기/펼치기**: 편지가 길 수 있어(네다섯 줄) 기본은 `line-clamp-2`로 2줄 미리보기, 탭하면 전체 펼침(다시 탭 접힘). `useRef`+`useEffect`로 접힘 상태 `scrollHeight>clientHeight` 측정해 **짧은 편지엔 "더보기" 미표시**.
