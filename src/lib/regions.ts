// 전국 행정구역 2단계 데이터: 시/도 → 시·군·구
// (세종특별자치시는 하위 시군구가 없는 단층제라 sigungu 가 빈 배열)
...
/** 저장된 지역 문자열("서울특별시 강남구" / "세종특별자치시")을 두 단계로 분해 */
export function parseRegion(region: string | null): {
  sido: string;
  sigungu: string;
} {
  if (!region) return { sido: "", sigungu: "" };
  const sido = SIDO_LIST.find(
    (s) => region === s || region.startsWith(s + " "),
  );
